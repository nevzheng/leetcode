...
void findMaxWidth(TreeNode* root, uint64_t x, int depth,
  ...
  if (!root) return;
  // First traversal to node stores the level. Traversing left then right,
  // guarantees the left most value is stored first
  levelMap.emplace(depth, x);
...
// For example the root of a subtree is at i, the left node is at 2*i, and the
// right node is at 2*i + 1. What we want to know is the difference between the
// position of the right most node and the left most node in the array. At Each
// node we already know the nodes position, but we need to know the Left Most
// Position. In order to have this info, we need to perform a preorder
// traversal. Storing the leftmost node in a hashmap, keyed by level When the
// right node is called, it should not be able to replace the left most value
// This will ensure that the left most value is ready when the right tree is
// called
using Level = uint64_t;
using LeftMost = uint64_t;
int widthOfBinaryTree(TreeNode* root) {
  uint64_t max_width = 0;
  map<Level, LeftMost> level_map;
  // The Root node is at i=0, in an array
  findMaxWidth(root, 0, 0, level_map, max_width);
